...
//	        this.debug_log(this.players[i].uid);
    ...
    dealLastThreePoker: function () {
        ...
        if (this.whoseTurn === 0) {
            ...
            const first = turnPlayer.findAPoker(54);
            for (var i = 0; i < 3; i++) {
                var p = this.tablePoker[i + 3];
                p.frame = 54;
                p.frame = 54;
    ...
    handleShotPoker: function (packet) {
        ...
        if (pokers.length === 0) {
            ...
            const gap = Math.min((this.game.world.width - PG.PW * 2) / count, PG.PW * 0.36);
    ...
    startCallScore: function (minscore) {
        ...
        if (this.whoseTurn === 0) {
            ...
            const ss = [1.5, 1, 0.5, 0];
            let sx = this.game.world.width / 2 - step * ss[minscore];
            const sy = this.game.world.height * 0.6;
            const group = this.game.add.group();
            const pass = this.game.make.button(sx, sy, "btn", btnTouch, this, 'score_0.png', 'score_0.png', 'score_0.png');
    ...
    createTableLayer: function (tables) {
        ...
        const gc = this.game.make.graphics(0, 0);
        ...
        for (let i = 0; i < tables.length; i++) {
            const sx = this.game.world.width * (i % 6 + 1) / (6 + 1);
            const sy = this.game.world.height * (Math.floor(i / 6) + 1) / (4 + 1);

            const table = this.game.make.button(sx, sy, 'btn', this.onJoin, this, 'table.png', 'table.png', 'table.png');
            table.anchor.set(0.5, 1);
            table.tableId = tables[i][0];
            group.add(table);

            const text = this.game.make.text(sx, sy, '房间:' + tables[i][0] + '人数:' + tables[i][1], style);
